...
typedef CHAR DOMAIN2           [ DOMAIN_LENGTH            + 1 ];
...
typedef struct _WFERECONNECTINFO {
   BOOL         bAddressValid; /* Name, Address, ClientName, and PortNum have valid data */
    /* Hack alert:  This field is only used to communicate across engine instances,
     *                   not within the engine */
#if defined(WIN32) && !defined(WINCE)
   UINT32      nRetriesRemaining;
#else
   BYTE         nRetriesRemaining;
#endif
...
// only valid for DesiredWinType==16
#define WFENGMONITOR_QUERY_LAYOUT_WINDOW		    1
#define WFENGMONTIOR_QUERY_LAYOUT_INITIALIZE	    2
...
// The following defines need to match those in XenDesktop\...\SecuritySettings.cs
// First two bits
